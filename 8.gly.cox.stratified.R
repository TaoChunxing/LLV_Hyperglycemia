#分层分析
#不同性别的LLV对Glygroup的影响
info.gender1 <- info.3  %>%  filter(Gender == 1) %>% select(1, 21)
info.gender2 <- info.3  %>%  filter(Gender == 2) %>% select(1, 21)

cox.stratf.gender1 <- info.gender1 %>% left_join(surv.hlme.glygroup.last, by = "ID")


#对hlme截至Flwtime第二次发生血糖异常
surv.match.hlme.gly0 <- match.hlme %>% filter(Glygroup==0)
surv.match.hlme.gly1 <- match.hlme %>% filter(Glygroup==1) %>% 
...
surv.match.hlme.glygroup <- bind_rows(surv.match.hlme.gly0, surv.match.hlme.gly1)
surv.match.hlme.glygroup.last <- surv.match.hlme.glygroup %>% 
  group_by(ID) %>% arrange(ID, Flwtime.m) %>% slice_tail(n = 1) %>% ungroup()
surv.match.hlme.glygroup.last$VLgroup <- as.factor(surv.match.hlme.glygroup.last$VLgroup)
surv.match.hlme.glygroup.last$VLgroup <- relevel(surv.match.hlme.glygroup.last$VLgroup, ref = "VS")
str(surv.match.hlme.glygroup.last)
table(surv.match.hlme.glygroup.last$VLgroup)
table(surv.match.hlme.glygroup.last$Glygroup)

write.xlsx(surv.match.hlme.glygroup.last, "8.0.surv.match.hlme.glygroup.last.xlsx")

#单因素cox回归获取HR值
cox <- coxph(Surv(Flwtime.m, Glygroup) ~ VLgroup, data = surv.match.hlme.glygroup.last)
cox
#计算HR(95%CI)
...
fit <- survfit(Surv(Flwtime.m,Glygroup) ~ VLgroup,
               data = surv.match.hlme.glygroup.last)
fit
